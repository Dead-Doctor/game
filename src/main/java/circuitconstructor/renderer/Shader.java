...
public class Shader {
    ...
    public Shader(String filepath) {
        ...
        try {
            ...
            else throw new IOException("Unexpexted token '" + firstPattern + "'");

            if (secondPattern.equals("vertex"))
                vertexSource = splitString[2];
            else if (secondPattern.equals("fragment"))
                fragmentSource = splitString[2];
            else throw new IOException("Unexpexted token '" + secondPattern + "'");

        } catch (IOException e) {
            e.printStackTrace();
            assert false : "Error: Could not open file path for shader: '" + filepath + "'";
        }
    }
